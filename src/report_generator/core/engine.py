...
from report_generator.utils.helpers import format_duration, generate_id
...
class Report:
    ...
    def __init__(
        ...
        Args:
            ...
            output_format: Format of the report (pdf, excel, html, json)
...
class ReportEngine:
    ...
    def generate(
        ...
        try:
            ...
            if output_format == "html":
                ...
                content = self._render_excel(combined_data)
            elif output_format == "json":
                content = self._render_json(combined_data)
            else:
    ...
    def _render_excel(self, data: pd.DataFrame | dict[str, pd.DataFrame]) -> bytes:
        """
        Render report data to Excel bytes.
        ...
        with pd.ExcelWriter(buffer, engine="openpyxl") as writer:
            if isinstance(data, pd.DataFrame):
                ...
                for idx, (name, df) in enumerate(data.items(), start=1):
                    sheet_name = (name or f"Sheet{idx}")[:31]
                    if not isinstance(df, pd.DataFrame):
                        df = pd.DataFrame(df)
                    df.to_excel(writer, index=False, sheet_name=sheet_name or f"Sheet{idx}")
            else:
                pd.DataFrame(data).to_excel(writer, index=False, sheet_name="data")

        return buffer.getvalue()

    def _render_json(self, data: Any) -> bytes:
        """
        Render report data to JSON bytes.

        Args:
            data: DataFrame or nested data to serialize

        Returns:
            JSON-encoded bytes
        """
        if isinstance(data, pd.DataFrame):
            serializable: Any = data.to_dict(orient="records")
        elif isinstance(data, dict):
            serializable = {
